...
ngx_http_limit_conn_module模块：基于共享内存，全部worker进程都会生效，限制的有效性基于key的设计，依赖postread阶段的realip模块取到的真实ip
...
Nix_http_limit_req_module模块：limit_req指令，限制并发连接数。

限制某些IP地址的访问权限：access模块
auth_basic模块：提供用户名密码访问权限
...
precontent阶段的try_files指令：nix_http_try_files_module模块。可以判断文件是否存在，不存在执行何种操作。
